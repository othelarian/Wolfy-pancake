...
fn get_prophetie() -> String {
  ...
  genProphetie(
    serde_wasm_bindgen::to_value(&trgs).unwrap(),
    serde_wasm_bindgen::to_value(&evts).unwrap()
  )
}
    ...
    <>
      <div class="lp-controls">
        ...
        <div class="lp-selector">
          <button class={sel1} onclick={sel1_cb}>{"1 prophecy"}</button>
          <button class={sel2} onclick={sel5_cb}>{"5 prophecies"}</button>
          <button class={sel3} onclick={sel10_cb}>{"10 prophecies"}</button>
        </div>
      </div>
      <hr />
      <div class="lp-propheties">
        if propheties_active.len() == 0 {
          <div class="lp-no-prophetie">
            {"The soothsayer will soon talk to you..."}
          </div>
        } else { {actives} }
      </div>
      <hr />
      if propheties_old.len() > 0 {
        <div><h4 class="lp-historic">{"Old prophecies"}</h4>{olds}</div>
      }
    </>
  }
}
